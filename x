...
-- Silent Precision time distortion setup
local OldTimeDistortion = shared.Forbidden['Silent Precision'].TimeDistortion

-- Smoothing parameters
local smoothingFactor = 0.1  -- Adjust for more/less smoothing (0 = no smoothing, 1 = instant)

-- Bullet speed (you need to adjust this to match your game mechanics)
local bulletSpeed = 300  -- Adjust the bullet speed according to your game mechanics

-- Previous velocity storage for smoothing
local previousVelocity = Vector3.new(0, 0, 0)

-- TriggerBot Settings from the table
local triggerBotSettings = shared.Forbidden['TriggerBot'].Settings
local safetySettings = triggerBotSettings['Preds']['Safety']
local shifts = triggerBotSettings['Shifts']
...
local function GetVelocity(player, part)
    if player and player.Character then
        ...
        -- Handle extreme velocities with the resolver setting
...
local function SmoothVelocity(currentVelocity)
    local smoothedVelocity = previousVelocity:Lerp(currentVelocity, smoothingFactor)
    previousVelocity = smoothedVelocity
    return smoothedVelocity
end

-- Function to predict future position based on velocity and bullet speed
local function PredictTargetPosition(targetPosition, targetVelocity, bulletSpeed, targetHeight)
    local distanceToTarget = (targetPosition - Camera.CFrame.Position).Magnitude
    local timeToImpact = distanceToTarget / bulletSpeed  -- Time for bullet to reach target
    
    -- Apply height-based offsets for prediction
    local verticalOffset = 0
    if targetHeight > 0 then
        verticalOffset = shifts.Air
    elseif targetHeight < 0 then
        verticalOffset = shifts.Drop
    else
        verticalOffset = shifts.Ground
    end

    -- Predict target's future position based on velocity, time to impact, and vertical offset
    local predictedPosition = targetPosition + targetVelocity * timeToImpact + Vector3.new(0, verticalOffset, 0)
...
-- Check if the cursor is inside the target's body hitbox
local function isCursorOnTarget(target)
    local screenPos = Camera:WorldToViewportPoint(target.Position)
    local mousePos = UserInputService:GetMouseLocation()
    
    local targetSize = target.Size * 2  -- Adjust size for better precision
    local targetMin = Vector2.new(screenPos.X - targetSize.X / 2, screenPos.Y - targetSize.Y / 2)
    local targetMax = Vector2.new(screenPos.X + targetSize.X / 2, screenPos.Y + targetSize.Y / 2)
    
    return (mousePos.X >= targetMin.X and mousePos.X <= targetMax.X and
            mousePos.Y >= targetMin.Y and mousePos.Y <= targetMax.Y)
end
    ...
    if TargetPlayer and TargetPlayer.Character then
        ...
        if humanoid and humanoid.Health > 0 then
            ...
            for _, part in pairs(bodyParts) do
                -- Get predicted velocity for the target's body part
                local targetVelocity = GetVelocity(TargetPlayer, part.Name)
                
                -- Smooth the predicted velocity
                local smoothedVelocity = SmoothVelocity(targetVelocity)
                
                -- Predict where the target will be based on velocity, bullet speed, and vertical shift
                local predictedPosition = PredictTargetPosition(part.Position, smoothedVelocity, bulletSpeed, part.Position.Y)
                
                -- Check if the cursor is inside the predicted target area
                if isCursorOnTarget(TargetPlayer.Character) then
                    local screenPos, onScreen = Camera:WorldToViewportPoint(predictedPosition)
                    if onScreen then
                        -- Sync the mouse position with the predicted target position
                        syncBoxWithTarget(screenPos)
                        if os.clock() - lastClickTime >= triggerBotSettings.ResetTime then
...
-- Handle triggerbot activation/deactivation with keybind
UserInputService.InputBegan:Connect(function(input, gameProcessed)
    if not gameProcessed and input.KeyCode == triggerBotSettings.Keybind then
        isToggled = true
    end
end)
...
-- Continuously update triggerbot action during the render step
