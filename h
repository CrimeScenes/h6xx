...
-- Function to predict the target player's position
local function predictTargetPosition(targetPlayer)
    ...
    if humanoid and targetPlayer.Character:FindFirstChild("HumanoidRootPart") then
        ...
        local acceleration = rootPart.AssemblyAngularVelocity
        local ping = targetPlayer:GetNetworkPing()

        -- Using velocity and simple kinematic equations to predict future position
        local predictionTime = ping * 2 / 1000 -- Convert ping to seconds and use a multiplier

        -- Basic prediction formula: position + velocity * time + 0.5 * acceleration * time^2
        local predictedPosition = rootPart.Position + velocity * predictionTime + 0.5 * acceleration * predictionTime^2
    ...
    if TargetPlayer and TargetPlayer.Character then
        ...
        if humanoid and humanoid.Health > 0 and not isPlayerKnocked(TargetPlayer) then
            ...
            if predictedPosition then
                ...
                -- Find the closest body part to the predicted position
                ...
                if bestTargetPart then
                    ...
                    -- Only fire when the target is within a reasonable range and the correct body part is targeted
...
-- Toggle triggerbot on key press
