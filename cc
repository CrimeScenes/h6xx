...
-- Check if target is within the viewable area
...
-- Get all body parts of the target
...
-- Sync the box with target (used for visuals, not critical to prediction)
...
-- Check if we are ignoring knife tools
...
-- Check if the mouse is on the target player
...
-- Improved function to predict the target player's future position
local function predictTargetPosition(targetPlayer)
    ...
    if humanoid and targetPlayer.Character:FindFirstChild("HumanoidRootPart") then
        local rootPart = targetPlayer.Character.HumanoidRootPart
        local velocity = rootPart.AssemblyLinearVelocity
        local previousVelocity = rootPart.AssemblyLinearVelocity
        local lastPosition = rootPart.Position
        local currentPosition = rootPart.Position
        local ping = targetPlayer:GetNetworkPing() / 1000 -- Convert ping to seconds

        -- Calculate velocity and acceleration based on past and current positions
        local velocityDiff = currentPosition - lastPosition
        local acceleration = velocityDiff / RunService.Heartbeat:Wait()  -- Assuming frame rate is constant
        local predictedPosition = currentPosition + velocity * ping + 0.5 * acceleration * ping^2
...
-- Triggerbot action (with hold mode and prediction)
local function TriggerBotAction()
    if TargetPlayer and TargetPlayer.Character then
        ...
        if humanoid and humanoid.Health > 0 and not isPlayerKnocked(TargetPlayer) then
            ...
            -- If the player is moving, use improved prediction
            if rootPart and rootPart.AssemblyLinearVelocity.Magnitude > 0 then
                predictedPosition = predictTargetPosition(TargetPlayer)
            end

            -- If the player is standing still, fire when mouse is on target
            if not predictedPosition and isMouseOnTarget(TargetPlayer) then
                local bodyParts = getBodyPartsPosition(TargetPlayer.Character)
                local closestDistance = math.huge
                local bestTargetPart = nil

                -- Find the closest body part to the mouse position
                for _, part in pairs(bodyParts) do
                    local screenPos, onScreen = Camera:WorldToViewportPoint(part.Position)
                    if onScreen and isWithinViewEra(part.Position) then
                        local distance = (Vector2.new(screenPos.X, screenPos.Y) - Vector2.new(getMousePosition())).Magnitude
                ...
                -- Fire only when within threshold distance and mouse is on the target
                if bestTargetPart and closestDistance <= shared.Forbidden.TriggerBot.Settings.Threshold then
                    local screenPos, onScreen = Camera:WorldToViewportPoint(bestTargetPart.Position)
                    syncBoxWithTarget(screenPos)

                    -- Fire if time to shoot
                    if os.clock() - lastClickTime >= shared.Forbidden.TriggerBot.Settings.ResetTime then
            ...
            -- If predicted position is available, aim and fire
            if predictedPosition then
                ...
                for _, part in pairs(bodyParts) do
                    local screenPos, onScreen = Camera:WorldToViewportPoint(part.Position)
                    local predictedScreenPos, onPredictedScreen = Camera:WorldToViewportPoint(predictedPosition)
                    if onScreen and onPredictedScreen and isWithinViewEra(predictedPosition) then
                        local distance = (Vector2.new(screenPos.X, screenPos.Y) - Vector2.new(predictedScreenPos.X, predictedScreenPos.Y)).Magnitude
                        if distance < closestDistance then
                            closestDistance = distance
                            bestTargetPart = part
                        end
                ...
                -- Fire only if the mouse is close to the predicted target body part
                if bestTargetPart then
                    local screenPos, onScreen = Camera:WorldToViewportPoint(bestTargetPart.Position)
                    syncBoxWithTarget(screenPos)

                    -- Fire only when the closest distance is within threshold
                    if closestDistance <= shared.Forbidden.TriggerBot.Settings.Threshold then
                        if os.clock() - lastClickTime >= shared.Forbidden.TriggerBot.Settings.ResetTime then
                            lastClickTime = os.clock()
...
-- Run the triggerbot action every frame while toggled
...
-- Continuous fire while holding the key, but only when target is within the prediction range
RunService.RenderStepped:Connect(function()
    if holdingKey then
        TriggerBotAction()
    end
