...
local function getTargetPrediction(target)
    local humanoidRootPart = target.Character and target.Character:FindFirstChild("HumanoidRootPart")
    if humanoidRootPart then
        local velocity = humanoidRootPart.Velocity
        local multiplier = shared.Forbidden.TriggerBot.Settings.Preds.PredictionMultiplier
        return humanoidRootPart.Position + (velocity * multiplier)
    end
    return nil
end

local function getShiftForState(target)
    local state = "Ground"
    if target.Velocity.Y > 0 then
        state = "Air"
    elseif target.Velocity.Y < 0 then
        state = "Drop"
    end
    local shifts = shared.Forbidden.TriggerBot.Settings.Shifts
    return shifts[state] or 0
end
    ...
    if TargetPlayer and TargetPlayer.Character then
        ...
        if humanoid and humanoid.Health > 0 and not isPlayerKnocked(TargetPlayer) then
            if isMouseOnTarget(TargetPlayer) then
                local predictedPosition = getTargetPrediction(TargetPlayer)
                if predictedPosition then
                    local shift = getShiftForState(TargetPlayer.Character)
                    local adjustedPosition = predictedPosition + Vector3.new(0, shift, 0)
                    local screenPos, onScreen = Camera:WorldToViewportPoint(adjustedPosition)
                    if onScreen and isWithinViewEra(adjustedPosition) then
